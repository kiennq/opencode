  ...
  export async function prune(input: { sessionID: string }) {
    ...
    for (const { part } of contentToPrune.parts) {
      if (part.state.status === "completed") {
        ...
        // Clear original output and large fields to save disk/memory
        part.state.output = ""
        part.state.attachments = undefined
        cleanupCompactedToolState(part.state)
...
---`
  ...
  /** Metadata fields to preserve for UI display */
  const PRESERVED_METADATA_KEYS = new Set([
    "sessionId", // task tool - child session navigation
    "summary", // task tool - child tool summary
    "diagnostics", // edit/write - LSP errors
    "todos", // todowrite - todo list display
    "answers", // question - answers display
    "count", // glob/grep - result count
    "matches", // grep - match count
    "preview", // read - code preview (first few lines)
  ])

  /** Input fields to preserve for UI display */
  const PRESERVED_INPUT_KEYS = new Set([
    "filePath", // edit/read/write - file path display
    "command", // bash - command display
    "pattern", // grep/glob - pattern display
    "content", // write - small, needed for display
    "description", // task - description display
  ])

  /**
   * Cleans up input and metadata of a compacted tool state in-place.
   * Called during prune() to minimize data stored on disk.
   */
  function cleanupCompactedToolState(state: MessageV2.ToolStateCompleted) {
    // Clear input except essential keys for display
    const cleanedInput: Record<string, unknown> = {}
    for (const key of PRESERVED_INPUT_KEYS) {
      if (key in state.input) cleanedInput[key] = state.input[key]
    }
    state.input = cleanedInput

    // Clear metadata except essential keys
    if (state.metadata) {
      const cleanedMeta: Record<string, unknown> = {}
      for (const key of PRESERVED_METADATA_KEYS) {
        if (key in state.metadata) cleanedMeta[key] = state.metadata[key]
      }
      // Special handling: truncate filediff to just show path
      if (state.metadata.filediff) {
        cleanedMeta.filediff = { path: state.metadata.filediff.path || state.metadata.filediff.file }
      }
      state.metadata = cleanedMeta
    }
  }

  /**
   * Aggressively cleans up parts from messages BEFORE the compaction point
   * to minimize disk and memory usage. This runs after compaction completes.
   */
  async function cleanupCompactedMessages(sessionID: string, messages: MessageV2.WithParts[]) {
    // Find the compaction point (message with CompactionPart)
    let compactionIdx = -1
    ...
    if (compactionIdx < 0) return // No compaction point found

    let cleanedParts = 0
    let deletedParts = 0
    const deletedPartIds = new Set<string>()

    // Clean up all messages BEFORE the compaction point
    for (let i = 0; i < compactionIdx; i++) {
      const msg = messages[i]
      for (const part of msg.parts) {
        const result = await cleanupPart(part, sessionID, msg.info.id)
        if (result === "cleaned") cleanedParts++
    ...
    // Remove deleted parts from in-memory messages array to prevent errors
    // when UI or other code tries to access them
    if (deletedPartIds.size > 0) {
      for (let i = 0; i < compactionIdx; i++) {
        const msg = messages[i]
        msg.parts = msg.parts.filter((p) => !deletedPartIds.has(p.id))
      }
    }

    if (cleanedParts > 0 || deletedParts > 0) {
      log.info("aggressive cleanup completed", { cleanedParts, deletedParts })
    }
  }

  async function cleanupPart(
    part: MessageV2.Part,
    sessionID: string,
    messageID: string,
  ): Promise<"cleaned" | "deleted" | "skipped"> {
    switch (part.type) {
      case "tool":
        return cleanupToolPart(part)

      case "reasoning":
        return cleanupReasoningPart(part)

      case "file":
        return cleanupFilePart(part)

      case "snapshot":
      case "patch":
        // These are only needed for revert - delete after compaction
        await Session.removePart({ sessionID, messageID, partID: part.id })
        return "deleted"

      case "step-start":
      case "step-finish":
        // Metadata parts - can be deleted after compaction
        await Session.removePart({ sessionID, messageID, partID: part.id })
        return "deleted"
  ...
  async function cleanupToolPart(part: MessageV2.ToolPart): Promise<"cleaned" | "skipped"> {
    if (part.state.status !== "completed") return "skipped"

    const state = part.state as MessageV2.ToolStateCompleted

    // Only clean up parts that were already compacted (have time.compacted set)
    // but may have been compacted before aggressive cleanup was added
    if (!state.time.compacted) return "skipped"

    // Check if cleanup is needed (has extra keys beyond preserved ones)
    const hasExtraInput = Object.keys(state.input).some((k) => !PRESERVED_INPUT_KEYS.has(k))
    const hasExtraMeta =
      state.metadata && Object.keys(state.metadata).some((k) => !PRESERVED_METADATA_KEYS.has(k) && k !== "filediff")

    if (!hasExtraInput && !hasExtraMeta) return "skipped"

    cleanupCompactedToolState(state)
    await Session.updatePart(part)
    return "cleaned"
  }

  async function cleanupReasoningPart(part: MessageV2.ReasoningPart): Promise<"cleaned" | "skipped"> {
    // Reasoning text is not needed after compaction - the summary captures what's important
    if (part.text && part.text.length > 100) {
      part.text = "[Reasoning cleared after compaction]"
  ...
  async function cleanupFilePart(part: MessageV2.FilePart): Promise<"cleaned" | "skipped"> {
    let changed = false

    // Clear source.text.value (large content) but keep start/end for highlighting
    if (part.source && "text" in part.source && part.source.text.value.length > 100) {
      part.source.text.value = "[Content cleared after compaction]"
      changed = true
    }

    // Truncate base64 data URLs (can be huge for images)
    if (part.url.startsWith("data:") && part.url.length > 1000) {
      // Keep just enough to show it was an image
      const mimeMatch = part.url.match(/^data:([^;,]+)/)
      part.url = mimeMatch ? `data:${mimeMatch[1]};cleared` : "data:cleared"
      changed = true
    }
