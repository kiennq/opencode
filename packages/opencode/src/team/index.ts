...
const MEMBER_TRANSITIONS: Record<MemberStatus, MemberStatus[]> = {
  ...
  shutdown: [],
  error: ["ready", "busy", "shutdown_requested", "shutdown"],
}
...
export namespace Team {
  ...
  export function onCleanedRestorePermissions(): () => void {
    return Bus.subscribe(TeamEvent.Cleaned, async (event) => {
      ...
      try {
        ...
        const sessionID = SessionID.make(event.properties.leadSessionID)
        await Session.setPermission({
          sessionID,
          permission: event.properties.delegate_permissions ?? [],
        })
  ...
  export const create = fn(
    ...
    async (input) => {
      ...
      const delegate_permissions = await (async () => {
        if (!input.delegate) return undefined
        try {
          const { Session } = await import("../session")
          const session = await Session.get(SessionID.make(input.leadSessionID))
  ...
  export async function setDelegate(teamName: string, delegate: boolean): Promise<void> {
    try {
      await Storage.update<TeamInfo>(configKey(teamName), (draft) => {
        ...
        if (delegate) draft.delegate_permissions ??= []
      })
    } catch {
  ...
  export async function setDelegatePermissions(
    teamName: string,
    permission: TeamInfo["delegate_permissions"],
  ): Promise<void> {
    try {
      await Storage.update<TeamInfo>(configKey(teamName), (draft) => {
        draft.delegate_permissions = permission
      })
    } catch {
      // Team not found — ignore
    }
  ...
  export async function resolveModel(input: {
    model?: string
    agent: { model?: { providerID: ProviderID; modelID: ModelID } }
    messages: Array<{ info: { role: string; model?: { providerID: ProviderID; modelID: ModelID } } }>
  }): Promise<{ providerID: ProviderID; modelID: ModelID } | { error: string }> {
  ...
  export async function approvePlan(input: {
    ...
    if (input.approved) {
      const sessionID = SessionID.make(member.sessionID)
  ...
  export async function cleanup(teamName: string): Promise<void> {
    ...
    await Bus.publish(TeamEvent.Cleaned, {
      ...
      delegate_permissions: team.delegate_permissions,
    })
  }
  ...
  export async function cancelMember(teamName: string, memberName: string): Promise<boolean> {
    ...
    const runtime = await SessionStatus.get(SessionID.make(member.sessionID))
  ...
  export async function recover(): Promise<{ interrupted: number }> {
    ...
    for (const team of teams) {
      ...
      try {
        const { TeamMessaging } = await import("./messaging")
        for (const member of active) {
          await TeamMessaging.recoverInbox(team.name, member.name, SessionID.make(member.sessionID))
        }
        await TeamMessaging.recoverInbox(team.name, "lead", SessionID.make(team.leadSessionID))
      } catch (err: unknown) {
      ...
      try {
        const { Session } = await import("../session")
        const sessionID = SessionID.make(team.leadSessionID)
        const msgs = await Session.messages({ sessionID })
        const lastUser = msgs.findLast((m) => m.info.role === "user")
        if (lastUser) {
          const info = lastUser.info as { agent: string; model: { providerID: ProviderID; modelID: ModelID } }
          const msgId = MessageID.ascending()
          await Session.updateMessage({
            id: msgId,
            sessionID,
