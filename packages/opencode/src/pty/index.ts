...
||||||| parent of dd6916434 (feat(timeline): add incremental loading and fix queued prompt matching)
    const id = PtyID.ascending()
    const command = input.command || Shell.preferred()
    const args = input.args || []
    if (command.endsWith("sh")) {
...
=======
    ...
    Bus.publish(Event.Created, { info })
    return info
>>>>>>> dd6916434 (feat(timeline): add incremental loading and fix queued prompt matching)
  }
