...
export namespace LSPServer {
  ...
  export const Typescript: Info = {
    ...
    async spawn(root) {
      const tsserver = await Process.resolve("typescript/lib/tsserver.js", Instance.directory)
  ...
  export const Vue: Info = {
    ...
    async spawn(root) {
      let binary = Process.which("vue-language-server")
      const args: string[] = []
      if (!binary) {
        const js = path.join(
          Global.Path.bin,
  ...
  export const ESLint: Info = {
    ...
    async spawn(root) {
      const eslint = await Process.resolve("eslint", Instance.directory).catch(() => {})
      if (!eslint) return
      log.info("spawning eslint server")
      const serverPath = path.join(Global.Path.bin, "vscode-eslint", "server", "out", "eslintServer.js")
      if (!(await File.exists(serverPath))) {
        if (Flag.OPENCODE_DISABLE_LSP_DOWNLOAD) return
        ...
        await File.write(zipPath, zipData)
        ...
        await Process.exec(`${npmCmd} install`, { cwd: finalPath, quiet: true })
        await Process.exec(`${npmCmd} run compile`, { cwd: finalPath, quiet: true })
  ...
  export const Oxlint: Info = {
    ...
    async spawn(root) {
      ...
      const resolveBin = async (target: string) => {
        ...
        if (await File.exists(localBin)) return localBin
      ...
      if (!lintBin) {
        const found = Process.which("oxlint")
      ...
      if (lintBin) {
        ...
        await proc.exited
        const help = proc.stdout ? await Util.streamToText(proc.stdout) : ""
      ...
      if (!serverBin) {
        const found = Process.which("oxc_language_server")
  ...
  export const Biome: Info = {
    ...
    async spawn(root) {
      ...
      if (await File.exists(localBin)) bin = localBin
      if (!bin) {
        const found = Process.which("biome")
        if (found) bin = found
      }
      ...
      if (!bin) {
        const resolved = await Process.resolve("biome", root).catch(() => undefined)
        if (!resolved) return
        bin = BunProc.which()
  ...
  export const Gopls: Info = {
    ...
    extensions: [".go"],
    async spawn(root) {
      let bin = Process.which("gopls", {
        PATH: process.env["PATH"] + path.delimiter + Global.Path.bin,
      })
      if (!bin) {
        if (!Process.which("go")) return
        if (Flag.OPENCODE_DISABLE_LSP_DOWNLOAD) return
  ...
  export const Rubocop: Info = {
    ...
    async spawn(root) {
      let bin = Process.which("rubocop", {
        PATH: process.env["PATH"] + path.delimiter + Global.Path.bin,
      })
      if (!bin) {
        const ruby = Process.which("ruby")
        const gem = Process.which("gem")
        if (!ruby || !gem) {
  ...
  export const Ty: Info = {
    ...
    async spawn(root) {
      ...
      for (const venvPath of potentialVenvPaths) {
        ...
        if (await File.exists(potentialPythonPath)) {
      ...
      if (!binary) {
        for (const venvPath of potentialVenvPaths) {
          ...
          if (await File.exists(potentialTyPath)) {
  ...
  export const Pyright: Info = {
    ...
    async spawn(root) {
      let binary = Process.which("pyright-langserver")
      const args = []
      if (!binary) {
  ...
  export const ElixirLS: Info = {
    ...
    async spawn(root) {
      let binary = Process.which("elixir-ls")
      if (!binary) {
        ...
        if (!(await File.exists(binary))) {
          const elixir = Process.which("elixir")
          if (!elixir) {
          ...
          await Process.exec("mix deps.get && mix compile && mix elixir_ls.release2 -o release", {
  ...
  export const Zls: Info = {
    ...
    async spawn(root) {
      let bin = Process.which("zls", {
        PATH: process.env["PATH"] + path.delimiter + Global.Path.bin,
      })

      if (!bin) {
        const zig = Process.which("zig")
        if (!zig) {
        ...
        const downloadData = new Uint8Array(await downloadResponse.arrayBuffer())
        await File.write(tempPath, downloadData)
        ...
        if (ext === "zip") {
          ...
          await Process.exec(`tar -xf ${tempPath}`, { cwd: Global.Path.bin, quiet: true, nothrow: true })
        }
        ...
        if (!(await File.exists(bin))) {
          log.error("Failed to extract zls binary")
          return
        }

        if (platform !== "win32") {
          await Process.exec(`chmod +x ${bin}`, { quiet: true, nothrow: true })
        }
  ...
  export const CSharp: Info = {
    ...
    extensions: [".cs"],
    async spawn(root) {
      let bin = Process.which("csharp-ls", {
        PATH: process.env["PATH"] + path.delimiter + Global.Path.bin,
      })
      if (!bin) {
        if (!Process.which("dotnet")) {
  ...
  export const FSharp: Info = {
    ...
    async spawn(root) {
      let bin = Process.which("fsautocomplete", {
        PATH: process.env["PATH"] + path.delimiter + Global.Path.bin,
      })
      if (!bin) {
        if (!Process.which("dotnet")) {
  ...
  export const SourceKit: Info = {
    ...
    async spawn(root) {
      ...
      const sourcekit = Process.which("sourcekit-lsp")
      if (sourcekit) {
        return {
      ...
      if (!Process.which("xcrun")) return

      const lspLoc = await Process.exec("xcrun --find sourcekit-lsp", { quiet: true, nothrow: true })
  ...
  export const RustAnalyzer: Info = {
    ...
    root: async (root) => {
      ...
      while (currentDir !== path.dirname(currentDir)) {
        ...
        try {
          const cargoTomlContent = await File.read(cargoTomlPath)
    ...
    extensions: [".rs"],
    async spawn(root) {
      const bin = Process.which("rust-analyzer")
      if (!bin) {
  ...
  export const Clangd: Info = {
    ...
    async spawn(root) {
      ...
      const fromPath = Process.which("clangd")
      if (fromPath) {
        return {
      ...
      const buf = new Uint8Array(await downloadResponse.arrayBuffer())
      if (buf.byteLength === 0) {
        log.error("Failed to write clangd archive")
        return
      }
      await File.write(archive, buf)
      ...
      if (tar) {
        await Process.exec(`tar -xf ${archive}`, { cwd: Global.Path.bin, quiet: true, nothrow: true })
      }
  ...
  export const Svelte: Info = {
    ...
    root: NearestRoot(["package-lock.json", "bun.lockb", "bun.lock", "pnpm-lock.yaml", "yarn.lock"]),
    async spawn(root) {
      let binary = Process.which("svelteserver")
      const args: string[] = []
      if (!binary) {
  ...
  export const Astro: Info = {
    ...
    root: NearestRoot(["package-lock.json", "bun.lockb", "bun.lock", "pnpm-lock.yaml", "yarn.lock"]),
    async spawn(root) {
      const tsserver = await Process.resolve("typescript/lib/tsserver.js", Instance.directory).catch(() => {})
      if (!tsserver) {
      ...
      let binary = Process.which("astro-ls")
      const args: string[] = []
      if (!binary) {
  ...
  export const JDTLS: Info = {
    ...
    extensions: [".java"],
    async spawn(root) {
      const java = Process.which("java")
      if (!java) {
        log.error("Java 21 or newer is required to run the JDTLS. Please install it first.")
        return
      }
      const javaMajorVersion = await Process.exec("java -version", { quiet: true, nothrow: true }).then(
        ({ stderr }) => {
      ...
      if (!installed) {
        ...
        const curlResult = await Process.exec(`curl -L -o ${archiveName} '${releaseURL}'`, {
          cwd: distPath,
          quiet: true,
          nothrow: true,
        })
        if (curlResult.exitCode !== 0) {
          log.error("Failed to download JDTLS", { exitCode: curlResult.exitCode, stderr: curlResult.stderr })
          return
        }

        log.info("Extracting JDTLS archive")
        const tarResult = await Process.exec(`tar -xzf ${archiveName}`, { cwd: distPath, quiet: true, nothrow: true })
        if (tarResult.exitCode !== 0) {
          log.error("Failed to extract JDTLS", { exitCode: tarResult.exitCode, stderr: tarResult.stderr })
          return
        }
      ...
      const jarFileName = await Process.exec("ls org.eclipse.equinox.launcher_*.jar", {
        cwd: launcherDir,
        quiet: true,
        nothrow: true,
  ...
  export const KotlinLS: Info = {
    ...
    async spawn(root) {
      ...
      const installed = await File.exists(launcherScript)
      if (!installed) {
        if (Flag.OPENCODE_DISABLE_LSP_DOWNLOAD) return
        ...
        await Process.exec(`curl -L -o '${archivePath}' '${releaseURL}'`, { quiet: true, nothrow: true })
        ...
        if (process.platform !== "win32") {
          await Process.exec(`chmod +x ${launcherScript}`, { quiet: true, nothrow: true })
        }
        log.info("Installed Kotlin Language Server", { path: launcherScript })
      }
      if (!(await File.exists(launcherScript))) {
  ...
  export const YamlLS: Info = {
    ...
    root: NearestRoot(["package-lock.json", "bun.lockb", "bun.lock", "pnpm-lock.yaml", "yarn.lock"]),
    async spawn(root) {
      let binary = Process.which("yaml-language-server")
      const args: string[] = []
      if (!binary) {
        const js = path.join(
          Global.Path.bin,
        ...
        const exists = await File.exists(js)
        if (!exists) {
          if (Flag.OPENCODE_DISABLE_LSP_DOWNLOAD) return
  ...
  export const LuaLS: Info = {
    ...
    extensions: [".lua"],
    async spawn(root) {
      let bin = Process.which("lua-language-server", {
        PATH: process.env["PATH"] + path.delimiter + Global.Path.bin,
      })
      ...
      if (!bin) {
        ...
        if (ext === "zip") {
          ...
          const ok = await Process.exec(`tar -xzf ${tempPath} -C ${installDir}`, { quiet: true })
            .then(() => true)
            .catch((error) => {
              log.error("Failed to extract lua-language-server archive", { error })
              return false
        ...
        if (platform !== "win32") {
          const ok = await Process.exec(`chmod +x ${bin}`, { quiet: true }).catch((error) => {
  ...
  export const PHPIntelephense: Info = {
    ...
    async spawn(root) {
      let binary = Process.which("intelephense")
      const args: string[] = []
      if (!binary) {
  ...
  export const Prisma: Info = {
    ...
    async spawn(root) {
      const prisma = Process.which("prisma")
      if (!prisma) {
  ...
  export const Ocaml: Info = {
    ...
    async spawn(root) {
      const bin = Process.which("ocamllsp")
      if (!bin) {
  ...
  export const BashLS: Info = {
    ...
    async spawn(root) {
      let binary = Process.which("bash-language-server")
      const args: string[] = []
      if (!binary) {
  ...
  export const TerraformLS: Info = {
    ...
    async spawn(root) {
      let bin = Process.which("terraform-ls", {
        PATH: process.env["PATH"] + path.delimiter + Global.Path.bin,
      })
  ...
  export const TexLab: Info = {
    ...
    async spawn(root) {
      ...
      if (!bin) {
        ...
        if (ext === "tar.gz") {
          await Process.exec(`tar -xzf ${tempPath}`, { cwd: Global.Path.bin, quiet: true, nothrow: true })
        }
  ...
  export const DockerfileLS: Info = {
    ...
    root: async () => Instance.directory,
    async spawn(root) {
      let binary = Process.which("docker-langserver")
      const args: string[] = []
      if (!binary) {
  ...
  export const Gleam: Info = {
    ...
    async spawn(root) {
      const gleam = Process.which("gleam")
      if (!gleam) {
  ...
  export const Clojure: Info = {
    ...
    async spawn(root) {
      let bin = Process.which("clojure-lsp")
      if (!bin && process.platform === "win32") {
        bin = Process.which("clojure-lsp.exe")
      }
      if (!bin) {
  ...
  export const Tinymist: Info = {
    ...
    async spawn(root) {
      let bin = Process.which("tinymist", {
        PATH: process.env["PATH"] + path.delimiter + Global.Path.bin,
      })
      ...
      if (!bin) {
        ...
        if (ext === "zip") {
          ...
          await Process.exec(`tar -xzf ${tempPath} --strip-components=1`, {
            cwd: Global.Path.bin,
            quiet: true,
            nothrow: true,
          })
  ...
  export const HLS: Info = {
    ...
    async spawn(root) {
      const bin = Process.which("haskell-language-server-wrapper")
      if (!bin) {
