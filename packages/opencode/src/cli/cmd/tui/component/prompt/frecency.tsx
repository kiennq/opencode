...
export const { use: useFrecency, provider: FrecencyProvider } = createSimpleContext({
  name: "Frecency",
  init: () => {
    const frecencyFilePath = path.join(Global.Path.state, "frecency.jsonl")
    onMount(async () => {
      const text = await File.read(frecencyFilePath).catch(() => "")
      const lines = text
        .split("\n")
        .filter(Boolean)
        .map((line) => {
      ...
      if (sorted.length > 0) {
        ...
        File.write(frecencyFilePath, content).catch(() => {})
      }
    })
    ...
    function updateFrecency(filePath: string) {
      ...
      appendFile(frecencyFilePath, JSON.stringify({ path: absolutePath, ...newEntry }) + "\n").catch(() => {})
