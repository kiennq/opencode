...
import { File as RuntimeFile } from "@opencode-ai/runtime"
...
export const EditTool = Tool.define("edit", {
  ...
  async execute(params, ctx) {
    ...
    await FileTime.withLock(filePath, async () => {
      if (params.oldString === "") {
        const existed = await RuntimeFile.exists(filePath)
        ...
        await RuntimeFile.write(filePath, params.newString)
      ...
      const stat = await RuntimeFile.stat(filePath).catch(() => undefined)
      ...
      contentOld = await RuntimeFile.read(filePath)
      ...
      await RuntimeFile.write(filePath, contentNew)
      await Bus.publish(File.Event.Edited, {
        file: filePath,
      })
      await Bus.publish(FileWatcher.Event.Updated, {
      ...
      contentNew = await RuntimeFile.read(filePath)
      diff = trimDiff(
        createTwoFilesPatch(filePath, filePath, normalizeLineEndings(contentOld), normalizeLineEndings(contentNew)),
      )
      FileTime.read(ctx.sessionID, filePath)
