...
function mockCtx(sessionID: SessionID) {
  return {
    sessionID,
...
describe("delegate mode cleanup restores permissions", () => {
  ...
  test("Team.cleanup preserves pre-existing write denies on lead session", async () => {
    await using tmp = await tmpdir({ git: true })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        try {
          const lead = await Session.create({})
          const name = uniqueName("delegate-preserve-write")
          ...
          await createTool.execute({ name, delegate: true }, mockCtx(lead.id))

          const cleanupTool = await TeamCleanupTool.init()
          await cleanupTool.execute({ name }, mockCtx(lead.id))

          const after = await Session.get(lead.id)
          expect(
            after.permission?.some((r) => r.permission === "bash" && r.pattern === "*" && r.action === "deny"),
          ).toBe(true)
          expect(
            after.permission?.some((r) => r.permission === "write" && r.action === "deny"),
            "delegate-only write deny should be removed",
          ).toBeFalsy()
        } finally {
          unsub()
        }
