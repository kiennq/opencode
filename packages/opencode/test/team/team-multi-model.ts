...
import { TeamCreateTool, TeamSpawnTool, TeamMessageTool } from "../../src/tool/team"
...
await Instance.provide({
  ...
  fn: async () => {
    ...
    const hasAnthropic = !!providers[ProviderID.anthropic]
    const hasGoogle = !!providers[ProviderID.google]
    const hasOpenAI = !!providers[ProviderID.openai]
    ...
    if (hasGoogle) {
      const models = Object.keys(providers[ProviderID.google].models)
    ...
    if (hasOpenAI) {
      const models = Object.keys(providers[ProviderID.openai].models)
    ...
    const teammateResults: Array<{ name: string; provider: string; model: string; sessionID: SessionID }> = []
    ...
    for (let i = 1; i < availableProviders.length; i++) {
      ...
      teammateResults.push({
        ...
        model: modelStr,
        sessionID: SessionID.make(result.metadata.sessionID as string),
      })
    }
