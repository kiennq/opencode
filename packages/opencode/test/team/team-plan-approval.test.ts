...
function mockCtx(sessionID: SessionID, messages: any[] = []) {
  return {
    sessionID,
  ...
  test("error: session not in any team", async () => {
    await Instance.provide({
      ...
      fn: async () => {
        const tool = await TeamApprovePlanTool.init()
        const result = await tool.execute(
          { name: "nobody", approved: true },
          mockCtx(SessionID.make("ses_orphan_" + Date.now())),
        )
...
describe("POST /team/:name/delegate route", () => {
  ...
  test("toggle off: preserves pre-existing write denies on lead session", async () => {
    await Instance.provide({
      directory: projectRoot,
      init: async () => {
        Env.set("ANTHROPIC_API_KEY", "test-key")
      ...
      fn: async () => {
        const name = uniqueName("route-delegate-preserve")
        const leadSession = await Session.create({})
        ...
        const session = await Session.get(leadSession.id)
        expect(
          session.permission?.some((r) => r.permission === "bash" && r.pattern === "*" && r.action === "deny"),
        ).toBe(true)
        expect(
          session.permission?.some((r) => r.permission === "write" && r.action === "deny"),
