...
async function seedUserMessage(sessionID: string | SessionID, text = "init") {
...
describe("Team.cancelMember", () => {
  ...
  test(
    "cancels an auto-woken teammate after they resume from ready",
    async () => {
      const server = Bun.serve({
        port: 0,
        async fetch() {
          await Bun.sleep(5000)
          return anthropicSSE("Working on the request")
        },
      })
      ...
      try {
        await Instance.provide({
          ...
          fn: async () => {
            ...
            await Team.create({ name: "cancel-auto-wake", leadSessionID: lead.id })
            await Team.addMember("cancel-auto-wake", {
              name: "worker",
              sessionID: member.id,
              agent: "general",
              status: "ready",
            ...
            const worker = team!.members.find((x) => x.name === "worker")!
            expect(worker.status).toBe("busy")
            expect(["starting", "running"]).toContain(worker.execution_status ?? "idle")
  ...
  test(
    "cancels an auto-woken teammate after they retry from error",
    async () => {
      const server = Bun.serve({
        port: 0,
        async fetch() {
          await Bun.sleep(5000)
          return anthropicSSE("Retrying the request")
        },
      })
      ...
      try {
        await Instance.provide({
          ...
          fn: async () => {
            ...
            await Team.create({ name: "cancel-auto-wake-error", leadSessionID: lead.id })
            await Team.addMember("cancel-auto-wake-error", {
              name: "worker",
              sessionID: member.id,
              agent: "general",
              status: "error",
            ...
            await TeamMessaging.send({
              teamName: "cancel-auto-wake-error",
              from: "lead",
              to: "worker",
