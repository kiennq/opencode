...
function mockCtx(sessionID: string | SessionID, overrides?: Partial<any>) {
  return {
...
async function seedUserMessage(sessionID: string | SessionID, text: string = "init", agent: string = "general") {
  ...
  test("create team, add tasks, spawn teammate (noReply), claim, complete, cleanup", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        await seedUserMessage(childSession.id, "You are researcher, a teammate. Research the auth module.", "explore")
  ...
  test("full spawn with SessionPrompt.loop() — teammate runs and goes idle", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        await seedUserMessage(leadSession.id, "init lead")
        ...
        await seedUserMessage(childSession.id, "You are auto-runner, a teammate. Research the auth module.")
        ...
        // Verify the loop completed — upstream loop() returns the final assistant message
  ...
  test("teammate-to-teammate messaging via lead", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        for (const sess of [sess1, sess2]) await seedUserMessage(sess.id)
  ...
  test("broadcast sends to all members except sender", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        for (const sess of [leadSession, sess1, sess2]) await seedUserMessage(sess.id)
