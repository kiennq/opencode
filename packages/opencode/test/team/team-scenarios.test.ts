...
function mockCtx(sessionID: string | SessionID, messages: any[] = []) {
  return {
  ...
  test("5 teammates spawned concurrently, all finish independently, no state corruption", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const allNotified = await waitFor(
          async () => {
            const msgs = await Session.messages({ sessionID: lead.id })
            return (
              msgs.filter((m) => m.parts.some((p) => p.type === "text" && p.text.includes("finished"))).length === 5
            )
          },
          15000,
          200,
          "all 5 idle notifications",
        )
        ...
        for (const member of team!.members) {
          const msgs = await Session.messages({ sessionID: SessionID.make(member.sessionID) })
