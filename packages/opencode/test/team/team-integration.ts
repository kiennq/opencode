...
function mockCtx(sessionID: string | SessionID, messages: MessageV2.WithParts[] = []) {
  return {
...
async function testConstraintEnforcement(leadSession: Session.Info) {
  ...
  // One team per lead
  const dupResult = await createTool.execute({ name: "dup-team" }, mockCtx(leadSession.id))
  ...
  const memberCreateResult = await createTool.execute({ name: "nested-team" }, mockCtx(memberSession.id))
  ...
  const memberShutdownResult = await shutdownTool.execute({ name: "someone" }, mockCtx(memberSession.id))
  ...
  const orphanResult = await orphanClaimTool.execute({ task_id: "t1" }, mockCtx(orphanSession.id))
...
async function testTeamSpawnWithRealLoop(leadSession: Session.Info) {
  ...
  const researcherSessionID = SessionID.make(spawnResult.metadata.sessionID as string)
  ...
  const idleNotification = leadMsgsAfter.find((m) =>
    m.parts.some(
      (p) => p.type === "text" && p.text.includes("[Team message from researcher]") && p.text.includes("finished"),
    ),
  )
...
async function testMultipleTeammatesConcurrent(leadSession: Session.Info) {
  ...
  const reviewerSessionID = SessionID.make(spawnReviewer.metadata.sessionID as string)
  const implementerSessionID = SessionID.make(spawnImplementer.metadata.sessionID as string)

  // Verify both registered
  const team = await Team.get("full-team")
  assert(
    team!.members.filter((m) => m.status === "busy" || m.status === "ready").length >= 2,
    "Multiple active/idle members",
  )
  ...
  const bothDone = await waitFor(
    ...
    90000,
    500,
    "reviewer and implementer to go idle",
  )
  ...
  const reviewerNotif = allLeadMsgs.find((m) =>
    m.parts.some(
      (p) => p.type === "text" && p.text.includes("[Team message from reviewer]") && p.text.includes("finished"),
    ),
  )
  const implementerNotif = allLeadMsgs.find((m) =>
    m.parts.some(
      (p) => p.type === "text" && p.text.includes("[Team message from implementer]") && p.text.includes("finished"),
    ),
  )
...
async function testMessaging(leadSession: Session.Info, teammateSessionIDs: Record<string, SessionID>) {
  ...
  const fromImplementer = leadMsgs.find((m) =>
    m.parts.some(
      (p) =>
        p.type === "text" && p.text.includes("[Team message from implementer]") && p.text.includes("token refresh"),
    ),
  )
...
async function testBroadcast(leadSession: Session.Info, teammateSessionIDs: Record<string, SessionID>) {
  console.log("\n========== 6. Broadcast ==========")
  ...
  for (const [name, sid] of Object.entries(teammateSessionIDs)) {
    ...
    const bcast = msgs.find((m) =>
      m.parts.some(
        (p) => p.type === "text" && p.text.includes("[Team message from lead]") && p.text.includes("New deadline"),
      ),
    )
  ...
  const selfBcast = leadMsgs.find((m) =>
    m.parts.some(
      (p) => p.type === "text" && p.text.includes("New deadline") && p.text.includes("[Team message from lead]"),
    ),
  )
  ...
  const selfBcast2 = await Session.messages({ sessionID: teammateSessionIDs.researcher }).then((msgs) =>
    msgs.filter((m) =>
      m.parts.some(
        (p) =>
          p.type === "text" &&
          p.text.includes("[Team message from researcher]") &&
          p.text.includes("auth spec updated"),
      ),
    ),
  )
...
async function testTaskCoordination(leadSession: Session.Info) {
  ...
  // List tasks via tool
  const listResult = await tasksTool.execute({ action: "list" }, mockCtx(leadSession.id))
  ...
  const doubleClaimResult = await claimTool.execute({ task_id: "t2" }, mockCtx(loserSid))
  assert(doubleClaimResult.title === "Claim failed", "Double claim via tool fails")

  // Cannot claim blocked task
  const blockedClaimResult = await claimTool.execute({ task_id: "t3" }, mockCtx(loserSid))
  ...
  const completeResult = await tasksTool.execute({ action: "complete", task_id: "t3" }, mockCtx(leadSession.id))
...
async function testShutdownAndCleanup(leadSession: Session.Info) {
  ...
  for (const member of activeMembers) {
    const result = await shutdownTool.execute({ name: member.name }, mockCtx(leadSession.id))
  ...
  const alreadyShutResult = await shutdownTool.execute({ name: team!.members[0].name }, mockCtx(leadSession.id))
  assert(alreadyShutResult.title === "Already shutdown", "Already shutdown member handled")

  // Shutdown tool on non-existent member
  const ghostResult = await shutdownTool.execute({ name: "ghost-member" }, mockCtx(leadSession.id))
  assert(ghostResult.title === "Error", "Non-existent member shutdown fails")

  // Cleanup
  const cleanupResult = await cleanupTool.execute({ name: "full-team" }, mockCtx(leadSession.id))
  ...
  const cleanupGhostResult = await cleanupTool.execute({ name: "ghost-team" }, mockCtx(leadSession.id))
