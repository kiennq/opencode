...
function hasToolCall(messages: MessageV2.WithParts[], toolName: string): boolean {
  return messages.some((m) =>
    m.parts.some(
      (p) => p.type === "tool" && "tool" in p.state && (p.state as any).input && (p as any).tool === toolName,
    ),
  )
}
...
async function testTeammateUsesToolsAutonomously(leadSession: Session.Info) {
  ...
  await createTool.execute(
    {
      ...
      tasks: [{ id: "check-schema", content: "Review the API schema for consistency issues", priority: "high" }],
    },
  ...
  const teammateSessionID = SessionID.make(spawnResult.metadata.sessionID as string)
  ...
  if (checkTask?.status === "completed") {
    ...
    assert(
      checkTask?.assignee === "schema-checker" || checkTask?.status !== "pending",
...
async function testTeammateClaimsUnblockedTask(leadSession: Session.Info) {
  ...
  await createTool.execute(
    {
      ...
      tasks: [
        ...
        {
          id: "build-on-top",
          content: "Build feature on top of foundation",
          priority: "high",
  ...
  assert(
    tasks.find((t) => t.id === "build-on-top")!.status === "pending",
    "build-on-top is pending after foundation completed",
  )
  ...
  const builderSessionID = SessionID.make(spawnResult.metadata.sessionID as string)
...
async function testTwoTeammatesCommunicate(leadSession: Session.Info) {
  ...
  const analyzerSessionID = SessionID.make(analyzerResult.metadata.sessionID as string)
  const reporterSessionID = SessionID.make(reporterResult.metadata.sessionID as string)
