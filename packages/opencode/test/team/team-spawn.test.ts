  ...
  test("basic spawn — child session has base deny rules", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const childSession = await Session.get(SessionID.make(result.metadata.sessionID))
  ...
  test("child session gets seeded user message with team context", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const childMsgs = await Session.messages({ sessionID: SessionID.make(result.metadata.sessionID) })
  ...
  test("spawned teammate is registered as active member", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const childSession = await Session.get(SessionID.make(member.sessionID))
        expect(childSession).toBeDefined()
