...
async function seedUser(sessionID: SessionID) {
...
describe("Team REST API routes", () => {
  ...
  describe("POST /session/:sessionID/team-message", () => {
    test("sends a teammate message through the session route", async () => {
      await Instance.provide({
        directory: projectRoot,
        init: async () => {
          Env.set("ANTHROPIC_API_KEY", "test-key")
        ...
        fn: async () => {
          const name = uniqueName("rt-session-team-message")
          const leadSession = await Session.create({})
          const memberSession = await Session.create({ parentID: leadSession.id })
          await seedUser(SessionID.make(memberSession.id))
          await Team.create({ name, leadSessionID: leadSession.id })
          await Team.addMember(name, {
            name: "reviewer",
            sessionID: memberSession.id,
          ...
          const app = Server.Default()
          const response = await app.request(`/session/${leadSession.id}/team-message`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ agent: "general", to: "reviewer", text: "Please review the diff" }),
          })
          ...
          const msgs = await Session.messages({ sessionID: SessionID.make(memberSession.id) })
          const injected = msgs.findLast((msg) => {
            const text = msg.parts.find((part) => part.type === "text")
            return text?.type === "text" && text.text === "[Team message from lead]: Please review the diff"
          })
